		...
		.then(ids => {
			db('users') 
				...
				.then(user => {
					...
					db.insert({user_id: user.id}).into('shopping_cart')
					.then(response => {
						return res.status(201).json({token, id: user.id, name: user.user_name})
					})
					.catch(err => {
						console.log(err)
						res.status(500).json({msg: 'error making cart for user'})
					})
				})
				.catch(err => {
					console.log(err)
